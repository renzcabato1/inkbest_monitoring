                        ...
                        <div class="content">
                            ...
                            <div class="module">
                                <div class="module-head">
                                    <h3>
                                      <?php echo"$branch";?> BRANCH SALES REPORT <td><?php
	echo'<a href="ink@best_print/sales_report_view_branch.php?from='.$from.'&to='.$to.'&branch='.$branch.'" target="_blank" class="btn btn-success">'?>Print</a></td></h3>
                                </div>
                                <div class="module-body table">
								<h4><?php echo"FROM: $from To $to"?></h4>
...
$sql2 = "SELECT * FROM invoice_history where branch='$branch' and date_invoice between '$from' and '$to'";
...
$sql3 = "SELECT DISTINCT inventory_id,price FROM inventory_invoice where  branch='$branch' and date_invoice between '$from' and '$to'";
$result3 = $conn->query($sql3);	
while($row3 = $result3->fetch_assoc()) {
	$id_mo=$row3['inventory_id'];
	$price=$row3['price'];
	$sql4= "SELECT SUM(qty) from inventory_invoice where inventory_id='$id_mo' and  branch='$branch'  and date_invoice between '$from' and '$to'";
...
while($row4 = $result4->fetch_assoc()) {
	...
	$sql6= "SELECT SUM(qty) from inventory_branch where  branch='$branch' and inventory_id='$id_mo'";
